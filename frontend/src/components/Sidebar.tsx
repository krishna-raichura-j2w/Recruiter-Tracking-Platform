...
import {
  ...
  UserCheck, ClipboardList, LogOut, Activity, Mail,
  ...
  admin: [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Mail Tracker',       path: '/mail-tracker',  icon: <Mail size={16} /> },
...
export default function Sidebar() {
  ...
  return (
    <div
      ...
      style={{ width: 248, minWidth: 248, background: 'linear-gradient(180deg, #0B1437 0%, #0F1B3F 100%)' }}
    >
      {/* Logo */}
      <div className="px-5 py-4 border-b border-white/8 flex items-center gap-3">
        <img src="/logo.jpg" alt="J2W" className="object-contain" style={{ maxHeight: 44, maxWidth: 160 }} />
      </div>

      {/* Role context chip */}
      <div className="mx-4 mt-4 mb-2 px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.05)' }}>
      ...
      <nav className="flex-1 px-3 py-2 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        {groups.map((group, gi) => (
          <div key={gi} className={gi > 0 ? 'mt-4' : ''}>
            {group.section && (
              <p className="text-[10px] font-bold uppercase tracking-widest px-3 mb-1.5" style={{ color: 'rgba(255,255,255,0.25)' }}>
                {group.section}
              </p>
            )}
                ...
                return (
                  <li key={item.path + item.label}>
                    <button
                      ...
                      onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).style.backgroundColor = 'rgba(255,255,255,0.07)'; (e.currentTarget as HTMLElement).style.color = '#FFFFFF'; }}

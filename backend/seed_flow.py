...
def upsert_validation(candidate_id, dl_id, status, validated_at, comments=None):
    ...
    if v:
        v.status = status
        v.delivery_lead_id = dl_id
        v.comments = comments
        v.updated_at = validated_at
    else:
        v = Validation(
            candidate_id=candidate_id,
...
CANDIDATE_FLOW = {
    ...
    19: (
        ...
        False,
        False,
        CandidateStatus.handed_to_recruiter,
    ),
    20: (
        nithish.id,
        shwetha.id,
...
for cand_id, (
    ...
    if caller_id:
        add_call_log(
            ...
            called_at,
            outcome="5_10min",
            notes="Profile matches JD requirements",
        )

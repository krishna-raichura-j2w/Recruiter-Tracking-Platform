        ...
        rows.append(
            {
                ...
                "skills": (
                    (a.primary_skill_stack or c.skills or "—")
                    if a
                    else (c.skills or "—")
                ),
...
def pod_report(
    ...
    jobs = (
        ...
        .order_by(Job.client_name, Job.role_title)
        .all()
    )

    # Lookup: user_id → user row (for sourcer/caller arrays)
    all_users = {u.id: u for u in db.query(User).filter(User.is_active).all()}
    ...
    L1_STAGES = {
        InterviewStage.l1_scheduled,
        InterviewStage.l1_feedback_pending,
        InterviewStage.l1_cleared,
        InterviewStage.l1_rejected,
        InterviewStage.l2_scheduled,
        InterviewStage.l2_feedback_pending,
        InterviewStage.l2_cleared,
        InterviewStage.l2_rejected,
        InterviewStage.final_scheduled,
        InterviewStage.final_feedback_pending,
        InterviewStage.final_cleared,
        InterviewStage.final_rejected,
        InterviewStage.offer_rolled_out,
        InterviewStage.offer_accepted,
    ...
    all_timelines = (
        ...
        .filter(SubmissionTimeline.stage == "submitted")
        .all()
    )
    ...
    def _pipeline(job_id: int) -> dict:
        ...
        submitted = sum(
            1 for c in cands if c.status in SUB_STATUSES or (c.submission is not None)
        )
        l1 = sum(
            1 for c in cands if c.submission and c.submission.current_stage in L1_STAGES
        )
        l2 = sum(
            1 for c in cands if c.submission and c.submission.current_stage in L2_STAGES
        )
        selections = sum(
            1
            for c in cands
            if c.submission and c.submission.current_stage in SEL_STAGES
        )
    ...
    def _cand_row(c: Candidate) -> dict:
        ...
        return {
            ...
            "notice_period_weeks": (
                _safe(a and a.notice_period_weeks, " wks") if a else "—"
            ),
            ...
            "current_stage": (
                sub.current_stage.value if sub and sub.current_stage else "—"
            ),
        }
    ...
    for r in recruiters:
        ...
        my_cands = [
            c for c in all_cands if c.sourced_by_id == r.id or c.assigned_to_id == r.id
        ]
        my_cands.sort(
            key=lambda c: (c.sourcing_date or "") if c.sourcing_date else "",
            reverse=True,
        )
        ...
        my_job_ids = [
            ...
            if r.id
            in (
                json.loads(j.sourcer_ids or "[]")
                if isinstance(j.sourcer_ids, str)
                else []
            )
            or r.id
            in (
                json.loads(j.caller_ids or "[]")
        ...
        recruiters_data.append(
            {
                ...
                "name": r.name,
                "recruiter_type": (
                    r.recruiter_type.value if r.recruiter_type else "both"
                ),

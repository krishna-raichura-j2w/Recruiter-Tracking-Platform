...
from features.mrr.jobs.schema import JobCreate, JobUpdate
...
def create_job(
    ...
    if is_kam and is_dl:
        ...
        if not kam_id:
            raise HTTPException(
                status_code=400,
                detail="A KAM must be selected when a Delivery Lead creates a JD.",
            )
        if not dl_ids:
        ...
        if not dl_ids:
            raise HTTPException(
                status_code=400,
                detail="At least one Delivery Lead must be selected before creating a JD.",
            )
        data["delivery_lead_id"] = dl_ids[0]
        created_by = current_user.id
    ...
    matched_client = (
        db.query(Client).filter(_f.lower(Client.name) == client_name.lower()).first()
    )
    if not matched_client:
        raise HTTPException(
            status_code=400,
            detail=f"Client '{client_name}' is not in the client list. Pick an existing client.",
        )
    if matched_client.client_id is None:
        raise HTTPException(
            status_code=400,
            detail=f"Client '{matched_client.name}' has no client_id set. Ask an admin to assign one before creating jobs for this client.",
        )
...
def update_job(
    ...
    if (
        "client_job_id" in data
        and data["client_job_id"]
        and service.is_job_id_taken(db, data["client_job_id"], exclude_job_id=job_id)
    ):
        raise HTTPException(
            status_code=400,
            detail=f"Job ID '{data['client_job_id']}' is already in use.",
        )
...
def delete_job(
    ...
    if not deleted:
        raise HTTPException(
            status_code=404,
            detail="Job not found or cannot be deleted — only your pending JDs can be deleted",
        )
    return {"message": "Deleted"}

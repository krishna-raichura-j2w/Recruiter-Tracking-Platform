...
def reassign_pod(
    ...
    if remaining:
        user.pod_lead_id = (
            remaining.pod_lead_id if remaining.pod_lead_id != to_dl_id else to_dl_id
        )
        ...
        "called": [
            {
                ...
                "status": (
                    log.candidate.status.value
                    if log.candidate and log.candidate.status
                    else None
                ),
                "job_title": (
                    log.candidate.job.role_title
                    if log.candidate and log.candidate.job
                    else None
                ),
                "client_name": (
                    log.candidate.job.client_name
                    if log.candidate and log.candidate.job
                    else None
                ),
...
def get_user_details(db: Session, user_id: int) -> dict | None:
    ...
    def _job_relation(j: Job) -> str:
        rels = []
        if j.created_by_id == user_id:
            rels.append("created")
        if j.delivery_lead_id == user_id:
            rels.append("DL")
        if j.assigned_sourcer_id == user_id:
            rels.append("sourcer")
        if j.assigned_caller_id == user_id:
            rels.append("caller")
        return ", ".join(rels) or "—"
    ...
    def _candidate_relation(c: Candidate) -> str:
        rels = []
        if c.sourced_by_id == user_id:
            rels.append("sourced")
        if c.assigned_to_id == user_id:
            rels.append("calling")
        if c.assigned_validator_id == user_id:
            rels.append("validating")
        return ", ".join(rels) or "—"
    ...
    return {
        "user": {
            ...
            "recruiter_type": (
                user.recruiter_type.value if user.recruiter_type else None
            ),

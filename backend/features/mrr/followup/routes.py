...
def _candidate_story(c: Candidate) -> dict:
    ...
    if c.call_logs:
        ordered = sorted(c.call_logs, key=lambda log: log.call_date or log.created_at)
        first = ordered[0]
    ...
    acknowledged_at = (
        _iso(m.acknowledgement_at) if (m and m.acknowledgement_received) else None
    )
...
def _job_story(db: Session, job: Job) -> dict:
    candidates = (
        ...
        .options(
            ...
            .joinedload(Submission.timeline)
    ...
    created_by = (
        db.query(UserModel).get(job.created_by_id) if job.created_by_id else None
    )
    return {
        "job_id": job.id,
...
def get_followup_jobs(
    ...
    for job in jobs:
        ...
        story["sourcer_names"] = [
            u.name for uid in s_ids if (u := db.query(UserModel).get(uid))
        ]
        story["caller_names"] = [
            u.name for uid in c_ids if (u := db.query(UserModel).get(uid))
        ]
        result.append(story)

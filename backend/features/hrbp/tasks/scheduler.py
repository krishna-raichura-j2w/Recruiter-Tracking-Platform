"""
HRBP scheduler — periodic jobs:

CURRENTLY ACTIVE:
  (none — all scheduler jobs are temporarily disabled; see notes below)

TEMPORARILY DISABLED:
1. check_sla_breaches (every minute)  [SLA DISABLED]
   Ticket-level: bump priority + escalate when overall SLA deadline passes.

2. check_step_sla_breaches (every minute)  [SLA DISABLED]
   Per-step: alert step owner + escalation manager when a step's SLA hours expire.

3. check_contract_closures (daily at 08:00 UTC)  [DISABLED]
...
# ── Ticket-level breach  [SLA DISABLED — uncomment to re-enable] ─────────────
#
...
# ── Per-step SLA breach  [SLA DISABLED — uncomment to re-enable] ─────────────
#
...
def start():
    ...
    # ── Ticket-level SLA breach check — SLA DISABLED ─────────────────────────
    # _scheduler.add_job(
    #     check_sla_breaches,
    #     "interval",
    #     minutes=1,
    ...
    # ── Per-step SLA breach check — SLA DISABLED ─────────────────────────────
    # _scheduler.add_job(
    ...
    # ── 4-month contract closure — DISABLED ──────────────────────────────────
    # _scheduler.add_job(
    ...
    _scheduler.start()
    log.info("HRBP scheduler started (all periodic jobs currently disabled).")

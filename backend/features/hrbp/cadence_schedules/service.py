...
from infra.hrbp_models import HRBPCadenceSchedule, HRBPCadenceSession, HRBPClient, HRBPConsultant
...
def create(db: Session, payload: CadenceScheduleCreate, hrbp_id: int) -> HRBPCadenceSchedule:
    ...
    session_dates = _generate_session_dates(payload.start_date, effective_end, payload.frequency_weeks)
    if not session_dates:
        raise HTTPException(status_code=400, detail="No valid weekday dates found in the given range")
...
def get_sessions(
    ...
    q = (
        ...
        .filter(HRBPCadenceSession.schedule_id == schedule_id)
    )
    if status is not None:
...
def get_session_by_id(db: Session, schedule_id: int, session_id: int) -> HRBPCadenceSession:
...
def get_summary(db: Session, hrbp_id: int | None = None) -> dict:
    base = (
        db.query(HRBPCadenceSession)
        .join(HRBPCadenceSchedule, HRBPCadenceSession.schedule_id == HRBPCadenceSchedule.id)
    )
    if hrbp_id is not None:
...
def list_all_sessions(
    ...
    rows = rows.order_by(HRBPCadenceSession.scheduled_date, HRBPCadenceSession.cadence_number)

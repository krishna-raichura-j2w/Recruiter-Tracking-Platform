...
async def extract_jd(
    ...
    if file:
        ...
        if mime == "application/pdf":
            parsed, cost, raw_text = service.extract_from_pdf(data)
        elif mime in IMAGE_MIMES:
            parsed, cost, raw_text = service.extract_from_image(data, mime)
        else:
            raise HTTPException(status_code=400, detail=f"Unsupported file type: {mime}")
    elif text and text.strip():
        parsed, cost, raw_text = service.extract_from_text(text.strip())
    else:

...
def _serialize_member(
    ...
    out = {
        **_user_short(u),
        ...
        "children": [_serialize_member(c, by_parent, team_by_dl) for c in by_parent.get(u.id, [])],
    }
...
def _build_tree(db: Session, pod: Pod) -> dict:
    ...
    members.sort(key=lambda u: (ROLE_ORDER.get(_user_role_value(u), 99), (u.name or "").lower()))
    by_parent: dict[int | None, list[User]] = {}
    for m in members:
    ...
    dl_ids = [m.id for m in members if _user_role_value(m) == UserRole.delivery_lead.value]
    ...
    if dl_ids:
        ...
        for dl_id, u in team_rows:
            ...
            if u.id in member_ids_set and _user_role_value(u) == UserRole.recruiter.value:
    ...
    if bh_user is not None:
        bh_node = {**_user_short(bh_user), "parent_user_id": None,
                   "children": [_serialize_member(c, by_parent, team_by_dl) for c in by_parent.get(bh_user.id, [])]}
...
def remove_member(
    ...
    if _user_role_value(user) == UserRole.delivery_lead.value:
        has_team = db.query(PodMembership).filter(PodMembership.pod_lead_id == user.id).first()
        if has_team:
            raise HTTPException(409, "This DL still has team members. Clear their team first.")
...
def dl_team_add(
    ...
    existing = db.query(PodMembership).filter(
        PodMembership.user_id == rec.id, PodMembership.pod_lead_id == dl.id
    ).first()
    if not existing:
...
def assignable_users(
    ...
    q = db.query(User).filter(User.role == role_enum, User.is_active == True)  # noqa: E712

#!/usr/bin/env python3
"""
Live OL Candidate Lookup Server.

Serves a browser UI that queries MRR Postgres + OL MySQL in real-time.

Usage:
    python scripts/ol_lookup_server.py            # default port 7070
    python scripts/ol_lookup_server.py --port 8888
"""
...
def _ser(v):
    if isinstance(v, (datetime, date)):
        return v.strftime("%d %b %Y %H:%M") if isinstance(v, datetime) else v.strftime("%d %b %Y")
    return v
...
def get_ol_data(email: str):
    ...
    try:
        with ol.cursor() as cur:
            ...
            user = serialize_row(dict(ol_user))
            uid  = ol_user["id"]
            ...
            """, (uid,))
            jobs = [serialize_row(dict(j)) for j in cur.fetchall()]
...
async function loadCandidates() {
  ...
  try {
    ...
    document.getElementById('cand-list').innerHTML =
      '<div style="padding:16px;color:#ef4444;font-size:12px">Failed to load candidates. Is the server running?</div>';
  } finally {
...
async function selectCandidate(email) {
  ...
  try {
    const res = await fetch('/api/ol/' + encodeURIComponent(email));
    ol = await res.json();
  } catch(e) {
    document.getElementById('detail').innerHTML =
...
if __name__ == "__main__":
    ...
    print(f"\n🚀  OL Lookup Server → http://localhost:{args.port}\n")
